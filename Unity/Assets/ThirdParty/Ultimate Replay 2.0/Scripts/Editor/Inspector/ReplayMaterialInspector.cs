...
#pragma warning restore 0414
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Display fields
            DisplayMultiEditableToggleField("Shared Material", "Should the shared material instance be recorded or the non-shared material instance", ref sharedMaterial);
            DisplayMultiEditableToggleField("Replay Color", "Record the material color", ref color);
            DisplayMultiEditableToggleField("Replay Main Texture Offset", "Record the uv offset of the main texture", ref mainTextureOffset);
            DisplayMultiEditableToggleField("Replay Main Texture Scale", "Record the scale of the main texture", ref mainTextureScale);
            DisplayMultiEditableToggleField("Replay Double Sided GI", "Record the double sided GI parameter of the material", ref doubleSidedGI);
            DisplayMultiEditableToggleField("Replay Global Illumination Flags", "Record the global illumination flags of the material", ref globalIlluminationFlags);
            DisplayMultiEditableToggleField("Interpolate", "Interpolate supported material properties such as material", ref interpolate);
        ...
        private void SetFlagValuesForTargets(ReplayMaterial.ReplayMaterialFlags flag, bool[] toggleValues)
        {
            for (int i = 0; i < targetMaterials.Length; i++)
            {
                if (toggleValues[i] == true) targetMaterials[i].recordFlags |= flag;
