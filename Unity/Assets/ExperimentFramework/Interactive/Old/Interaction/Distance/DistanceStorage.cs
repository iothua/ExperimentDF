...
namespace ExperimentFramework.Interactive.Distance
{
    ...
    public static class DistanceStorage
    {
        ...
        public static void AddDistanceData(List<DistanceDataManager> managers, DistanceInteraction data)
        {
            switch (data.distanceData.interactionType)
            {
                ...
                case InteractionType.Send:
                    ...
                    {
                        if (item.InteractionObject.Equals(data.InteractionObject)) continue;
                ...
                case InteractionType.Receive:
                    ...
                    for (int i = 0; i < managers.Count; i++)
                    {
                        ...
                        if(!manager.sendData.distanceData.TagID.Equals(data.distanceData.TagID)) continue;

                        manager.AddDistance(data);
                    }
        ...
        public static List<DistanceDataManager> GetSendDistaceDataAll(DistanceInteraction data, InteractionType interactionType)
        {
            ...
            {
                var obj = dataManagers[i];
                if(obj==null) continue;
                if(obj.sendData.InteractionObject==null) continue;
                if (obj.sendData != null && obj.sendData.distanceData.TagID.Equals(data.distanceData.TagID)
                                         && obj.sendData.distanceData.interactionType.Equals(interactionType)
                                         && !obj.sendData.Equals(data)
                                         && !obj.sendData.InteractionObject.Equals(data.InteractionObject))
                {
                    managers.Add(obj);
                }
            }
            
            // return dataManagers.FindAll(obj => obj != null && (obj.sendData != null && obj.sendData.distanceData.TagID.Equals(data.distanceData.TagID)
            //                                                                         && obj.sendData.distanceData.interactionType.Equals(interactionType)
            //                                                                         && !obj.sendData.Equals(data)
            //                       
            //                                                                         && !obj.sendData.InteractionObject.Equals(data.InteractionObject)));
        ...
        public static List<DistanceDataManager> GetSendDistaceDataKey(DistanceInteraction data)
        {
            ...
            for (int i = 0; i < dataManagers.Count; i++)
            {
                ...
                if (obj.sendData.distanceData.TagID.Equals(data.distanceData.TagID) && obj.sendData.Equals(data))
                {
                    managers.Add(obj);
                }
            }
        ...
        public static List<DistanceDataManager> GetSendDistanceData(InteractionType type)
        {
            ...
            for (int i = 0; i < dataManagers.Count; i++)
            {
                ...
                if ( obj.sendData.distanceData.interactionType.Equals(type))
                {
                    managers.Add(obj);
                }
            }
        ...
        public static List<DistanceInteraction> GetReceiveDistanceData(string TagID)
        {
            ...
            for (int i = 0; i < dataManagers.Count; i++)
            {
                ...
                for (int j = 0; j < obj.Distances.Count; j++)
                {
                    if(obj.Distances[j]==null) continue;
                    if (obj.Distances[j].distanceData.TagID.Equals(TagID))
                    {
                        distances.Add(obj.Distances[j]);
                    }
                }
            }
        ...
        public static DistanceDataManager GetSendDistanceManager(DistanceInteraction data)
        {
            ...
            for (int i = 0; i < dataManagers.Count; i++)
            {
                ...
                if (obj.sendData != null && obj.sendData.Equals(data))
                {
                    return obj;
                }
            }
        ...
        public static bool IsExistDistanceData(List<DistanceDataManager> dataManagers, DistanceInteraction data)
        {
            ...
            for (int i = 0; i < dataManagers.Count; i++)
            {
                ...
                if (obj.sendData.Equals(data)) return true;
            }
        ...
        public static void Merge(this List<DistanceDataManager> dataManagers, List<DistanceDataManager> distances)
        {
            ...
            for (var index = 0; index < distances.Count; index++)
            {
                ...
                DistanceDataManager manager = null;
                
                for (int i = 0; i < dataManagers.Count; i++)
                {
                    if(dataManagers[i]==null) continue;
                    if (!dataManagers[i].sendData.Equals(distance.sendData)) continue;
                    manager = dataManagers[i];
