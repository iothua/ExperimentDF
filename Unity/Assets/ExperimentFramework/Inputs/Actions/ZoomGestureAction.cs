...
{
    public class ZoomGestureAction : GestureAction
    {
        ...
        public ZoomGestureAction()
        {
            ScaleMultiplier = ScaleMultiplierX = ScaleMultiplierY = 1.0f;
            
#if UNITY_2017_4_OR_NEWER

            ZoomSpeed = (Input.mousePresent ? 3.0f : 1.0f);
        ...
        public void ProcessTouches()
        {
            ...
            float distance = DistanceBetweenPoints(Input.touches[0].position.x, Input.touches[0].position.y, Input.touches[1].position.x, Input.touches[1].position.y);
            float distanceX = Distance(Input.touches[0].position.x - Input.touches[1].position.x);
            float distanceY = Distance(Input.touches[0].position.y - Input.touches[1].position.y);
            ...
            else if (State == GestureRecognizerState.Executing)
            {
                ...
                if (distance != previousDistance)
                {
                    ...
                    // 检查我们是否高于抖动阈值-如果沿与上次相同的方向移动，将始终为true
        ...
        public override void TouchMoved()
        {
            ProcessTouches();
        }
